...
class Keccak:
    ...
    def update(self, arg):
        ...
        # m.update(a); m.update(b) is equivalent to m.update(a+b). arg is a normal
        # bytestring.
        ...
        if len(self.buffered_data) * 4 >= self.r:
            ...
            if extra_bits == 0:
                ...
                # Slice it up into the first r*a bits, for some constant a>=1, and the
                # remaining total-r*a bits.
            ...
            for i in xrange((len(P) * 8 // 2) // self.r):
                to_convert = P[
                    i * (2 * self.r // 8):(i + 1) * (2 * self.r // 8)] + '00' * (self.c // 8)
    ...
    def digest(self):
        ...
        assert len(self.buffered_data) == 0, "Why is there data left in the buffer? %s with length %d" % (
            self.buffered_data, len(self.buffered_data) * 4)
...
if __name__ == '__main__':
    ...
    rounds = 1000
    data = 'test' * 8 * 8
    print 'size', len(data), 'rounds', rounds
...
"""
pypy
sha3 [62825, 104594396, 13922, 391.85162568092346]
=================================== 824 passed in 549.21 seconds ===================================
py.test -s tests/test_state.py --nocapturelog  513,93s user 22,68s system 97% cpu 9:09,74 total


160 seconds w/o sha3

--------
py
sha3 [62825, 104595337, 13922, 0.7603144645690918]
=================================== 824 passed in 152.96 seconds ===================================
py.test -s tests/test_state.py --nocapturelog  146,18s user 4,24s system 97% cpu 2:33,51 total


------------------

sha3 [673770, 54194423, 71432, 570.2660627365112]
.

================================== 31778 passed in 810.07 seconds ==================================
py.test -s tests/test_vm.py --nocapturelog  772,95s user 14,84s system 97% cpu 13:31,03 total
...
sha3 [673770, 54194423, 71432, 3.146991729736328]
================================== 31778 passed in 235.30 seconds ==================================
py.test -s tests/test_vm.py --nocapturelog  216,85s user 6,29s system 94% cpu 3:57,31 total
